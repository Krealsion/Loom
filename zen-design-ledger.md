...
the `authorize_connection` chokepoint returns full grant today; *B* is a bearer capability token
(possession-is-authorization, no identity), *C* is per-connection graduated/differential grants,
**where "Weaver" is born** (the first relation needing differential persistent authority = the first
place authorization needs *authentication* = the identity phase). *Location-transparent addressing* —
contract-addressing pointed at *location* (the same contract-address resolving to a different host),
the one refinement **relocation** pulls so senders resolve a relocated weave's new location directly
rather than via a forwarding-proxy hop (relocation otherwise falls out for free on the proxy-participant
pattern). *The socket-frame `TerminalBackend`* (host-rendered output over the wire) — hooked by the new
`write`/`flush` seam, no consumer yet. *A neutral `wire.hpp`* sharing the two protocols' framing
primitives. **Multi-threaded dispatch stays explicitly deferred** — the bridge's multiplexer is the
*client's* readiness, never the bus's concurrency; conflating them would be a real error.
