...
**Status: Stage 2 built** (the engine + a plain terminal; the dataflow layer on top). The
...
its resolved type exactly. Still designed, not built: the live-redraw flipbook rendering / UI-as-data
TUI. Named successor: **Stage 3** (the UI-as-data TUI — intent and relationship, never absolute
position/size).
