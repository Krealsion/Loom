...
| R2E-0 | **what I know, what I become.** Three closures and one proof-of-no-API. (1) **Senses**: a deliberate immutable *latest claim*, visible at the claim call — the smallest settlement rule, and indistinguishable from handler-completion because dispatch is non-reentrant. By value (no alias into a claimant), two key spaces so personal ≠ office is structural, stale office claims *stamped* rather than withheld (returning nothing would collapse "never claimed" and "the predecessor's"), a new default-absent **observe rule** so no weave gained reach, `Claims<...>` registering at mount for discovery-before-first-claim, and a repository bounded by current keys. **ABI v6** earned: a host-only Sense is the feature not existing. (2) **Authored handoff**: proven to need **no Loom API** — prepared replacement, FIFO, `NotAccepted` and an ordinary weave already supply every piece; migration is an authored transformation *before* admission, never coercion inside the gate; the namespace witness runs carried and deliberately-not-carried. (3) **MSG-08**: the Night Lab III silent dynamic seam closed with one Loom-owned refusal fact and no delivery futures. (4) **MSG-09**: a bounded dispatch turn earned from the Rule Garden's fake yield message; `pump()` untouched | Loom `101ebb9`,`7e189a0`,`ae9c0e6`,`c281985` | [SENSE-01..05](../laws/sense-laws.md), [HANDOFF-01..03](../laws/handoff-laws.md), [MSG-08/09](../laws/messaging-laws.md), [a-claim-is-not-a-message](../decisions/a-claim-is-not-a-message.md), [migration-is-authored-not-inferred](../decisions/migration-is-authored-not-inferred.md), [dynamic-abi v6](../reference/dynamic-abi.md) |

| R2E-0a | **making R2E-0's claims exact.** A correction pass, not a phase: the experimental sequence was reconstructed onto the neutral baseline and then four things it got wrong were fixed. (1) The dynamic Sense seam **truncated office identity** at a 128-byte buffer, so a long role came back as a plausible prefix nobody authored — the bound is removed, not raised, and the name crosses through the same caller-owned sink the value already used. (2) A reading answered only whether the author's **life** was current; a live replacement moves the *incarnation* while the life stands, so `author_incarnation_is_current` is now asked separately and a predecessor's still-valid claim is distinguishable from the current incarnation's. (3) The **numeric bound was withdrawn** — `pump_bounded(n)` / `set_dispatch_budget(n)` were public API the consumer had already disproved (64 throttled the Rule Garden 17×), and no independent consumer wanted a message budget; `pump_pending()` is the only bounded turn. (4) Prose corrections: root/host `claim_as` named rather than implied absent, the migrator presented as a tradeoff rather than a necessity, stale numeric-budget language removed from current-facing docs. **ABI stays v6** — it was never published, so its layout changed without a compatibility boundary being crossed | Loom `0412015`,`1361b03`,`b6c51e6` | [MSG-09 corrected](../laws/messaging-laws.md), [SENSE-01..05](../laws/sense-laws.md), [known-seams](../reference/known-seams.md), [dynamic-abi v6](../reference/dynamic-abi.md) |
