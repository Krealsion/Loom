...
| OBSERVATION — one office's publications told live to a subscriber a host admits (the relay `loom.observe` and its host policy, readiness, holder-only telling, `cause` from a fence the subscriber opened, the window, `Gap` and `latest` coalescing, the four endings, a link's custody and `lost`, the Python `Subscription`), and a tool package that BUILDS ON another (`uses`) | reference/observation.md, guides/observing.md#following-another-participants-publications, reference/bounds.md#observation-relay, guides/sessions.md#a-package-that-builds-on-another, ../include/zen/observe/relay.hpp | MSG-02, ANS-05, LIFE-08 | tests/test_observe.cpp, tests/test_bridge.cpp (the `link:` cases on far observations), tests/test_runs.cpp (the `uses` cases), tests/session/test_client.py (O1..O11), tests/session/journey.py (L6) | the ELH event-driven-observation phase |
