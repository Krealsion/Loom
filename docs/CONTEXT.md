...
| a HOST that must serve a bus and a person at once (the bounded turn, why an unbounded drain loses the operator to a cooperative weave that re-arms itself, and why a conversation that has not settled is reported as pending rather than completed) | guides/running-loom.md#9-work-that-starts-by-itself-and-work-that-never-stops, ../src/host/line_input.hpp | MSG-09 | tests/host_process/run.cmake | FRIC-1, and one approved self-addressed message |
