...
| a HOST that must serve a bus and a person at once (the bounded turn, why an unbounded drain loses the operator to a cooperative weave that re-arms itself, why a conversation that has not settled is reported as pending rather than completed, and why the wait for a line must hold while one is half typed — and a Windows console is therefore read on its own thread) | guides/running-loom.md#9-work-that-starts-by-itself-and-work-that-never-stops, ../src/host/line_input.hpp | MSG-09 | tests/host_process/run.cmake, tests/host_terminal/witness.cpp | FRIC-1, one approved self-addressed message, and a half-typed command on a Windows console |
