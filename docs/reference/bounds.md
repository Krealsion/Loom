...
| `RunManager::kShutdownObserveMs` | 2000 ms | the WHOLE of a clean shutdown's budget for watching the executions it just stopped actually end, so that a final `exited`/`killed` carries an exit code the manager read. Unspent time is not waited out; what is not seen in it is recorded `killing`, which promises no code |
