...
// A Weave that, when poked with a Go, asks the kernel to load a library by
...
class Loader : public au::WeaveBase<Loader, LoaderState, au::Accept<Go>,
                                    au::Emit<loom::LoadLibrary>> {
public:
    WeaveId control{};
    std::string lib_name;
    std::string lib_path;
...
TEST_CASE("a send the grant does not permit is denied before the gate, and observed") {
    ...
    sender.weave->on_handle = [rid = recorder.id](const Message&, Bus& bus_, ProbeWeave&) {
    ...
    CHECK(recorder.weave->handled_names.empty()); // Pong never delivered
...
TEST_CASE("publish only reaches the accepters the sender is permitted to send to") {
    ...
    pub.weave->on_handle = [](const Message&, Bus& bus_, ProbeWeave&) {
...
TEST_CASE("the kernel door is driven by message only with the load capability") {
    ...
    WeaveId with_cap = au::mount_granted<Loader>(bus, load_capability(control));
    auto* lw = static_cast<Loader*>(bus.weave(with_cap));
    lw->control = control;
    lw->lib_name = "viamsg";
    ...
    // never loads.
    WeaveId no_cap = au::mount_granted<Loader>(bus, Grant::nothing());
    auto* ln = static_cast<Loader*>(bus.weave(no_cap));
    ln->control = control;
    ln->lib_name = "denied";
    ln->lib_path = ZEN_SO_WEAVE;
