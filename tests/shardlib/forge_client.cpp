...
// Mail::send_to_role (which hard-codes reply_to = ShardId{}, so an honest mod can forge nothing).
// Instead it reaches the raw bus (mail.bus()) and emits a role-send StorageGet whose WIRE reply_to
// is forged to point at a DIFFERENT Shard (the `victim` carried in the trigger). The host must
...
class ForgeClient
    : public ShardBase<ForgeClient, ForgeState, Accept<DoForge, StorageValue>, Emit<StorageGet>> {
public:
    void on(const DoForge& m, Mail& mail) {
        ...
        zen::sb::Message forged(to_value(StorageGet{m.key}), zen::sb::ShardId{},
                                zen::sb::ShardId{static_cast<std::uint64_t>(m.victim)}, 0);
