...
// those shapes to the control Weave is the **load capability** — the canonical
// dangerous grant. A Weave holding it can drive the kernel by message; a Weave
...
/// A Weave whose handlers drive a Kernel. It emits nothing (its authority is its
/// accept-set being reachable, gated by the *sender's* load capability).
class ControlWeave
    : public loom::WeaveBase<ControlWeave, ControlState,
                                    loom::Accept<LoadLibrary, ReloadLibrary, UnloadLibrary>> {
public:
    explicit ControlWeave(Kernel& kernel) : kernel_(&kernel) {}

    void on(const LoadLibrary& m, loom::Mail&) {
        ++state_.ops;
        (void)kernel_->load(m.name, m.path);
    }
    void on(const ReloadLibrary& m, loom::Mail&) {
        ++state_.ops;
        (void)kernel_->reload_from(m.name, m.path);
    }
    void on(const UnloadLibrary& m, loom::Mail&) {
        ++state_.ops;
...
/// Register the control Weave on `bus` and return its id. It emits nothing, so it
...
/// The grant that lets a Weave drive the kernel: permission to send the three
/// control shapes to the control Weave, and only to it.
