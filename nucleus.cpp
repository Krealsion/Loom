...
//     cannot construct a message for a Weave whose type was never
...
static std::shared_ptr<loom::Schema> schema(std::string name, std::vector<loom::Field> fields) {
    auto s = std::make_shared<loom::Schema>();
    s->name = std::move(name);
    s->fields = std::move(fields);
    return s;
}
static loom::Field f(std::string n, loom::Kind k) { return { std::move(n), k, nullptr, true }; }

static loom::Cell ci(std::int64_t x) { return x; }
static loom::Cell cf(double x)       { return x; }
static loom::Cell ct(std::string x)  { return x; }
static loom::Cell cb(bool x)         { return x; }

// --- a recursive printer so values can be seen -----------------------------
static void print_value(const loom::Value& v, int indent) {
    ...
    for (const auto& [key, c] : v.cells) {
        ...
        std::visit([&](auto&& x) {
            ...
            if constexpr (std::is_same_v<T, std::shared_ptr<loom::Value>>) {
                std::cout << "\n";
...
static void report(const loom::Admission& a) {
...
static loom::Value construct_blind(std::shared_ptr<const loom::Schema> shape,
                                  const std::function<loom::Cell(const loom::Field&)>& source) {
    loom::Value v;
    v.schema = shape;
    ...
    for (const auto& fld : shape->fields) {
        std::cout << "    console:   field '" << fld.name << "' : "
                  << loom::name_of(fld.kind) << "\n";
        v.cells[fld.name] = source(fld);
    }
    return v;
}

// --- a Weave that declares how it wishes to return -------------------------
struct Weave {
    std::string name;
    std::shared_ptr<const loom::Schema> state_schema; // the lock the self chose
    loom::Value policy;                                // self-declared; kernel-checked
    loom::Value last_known_good;                       // the safe self to revert to
    std::int64_t reloads_used = 0;
};

static void revive(Weave& s, const loom::Value& candidate, const loom::Schema& policy_door) {
    ...
    auto pol = loom::admit(s.policy, policy_door);
    ...
    auto a = loom::admit(candidate, *s.state_schema); // the self-set lock
    if (a.ok) {
...
int main() {
    ...
    loom::Value step; step.schema = Move;
    step.cells["dx"] = cf(1.0); step.cells["dy"] = cf(-2.0);
    std::cout << "  a well-formed Move, sent across the bus:\n";
    report(loom::admit(step, *Move));

    loom::Value broken; broken.schema = Move;
    broken.cells["dx"] = cf(1.0); // dy never set
    std::cout << "  a Move that lost a field:\n";
    report(loom::admit(broken, *Move));

    auto Spawn = schema("Spawn", { f("kind", Kind::Text) });
    loom::Value liar; liar.schema = Spawn; liar.cells["kind"] = ct("dragon");
    std::cout << "  a Spawn, offered to a door that admits only Move:\n";
    report(loom::admit(liar, *Move));

    // -- B. The console builds a message for a Weave it has never seen ------
    std::cout << "\nB. The console builds a message for a Weave it has never seen.\n\n";
    ...
    auto source = [](const loom::Field& fld) -> loom::Cell {
    ...
    loom::Value built = construct_blind(SetColor, source);
    std::cout << "  the console produced:\n";
    print_value(built, 2);
    std::cout << "  offered to the SetColor Weave's threshold:\n";
    report(loom::admit(built, *SetColor));

    // -- C. The self sets the lock its own return must open -----------------
    std::cout << "\nC. The self sets the lock its own return must open.\n\n";

    // The kernel has NO compiled ReloadPolicy type. A policy is just a value
    // against a shape the kernel was handed. Here a Weave declares one.
    ...
    report(loom::admit(ami.policy, *ReloadPolicy));

    std::cout << "\n  death 1 — a clean save returns:\n";
    loom::Value good; good.schema = PlayerState;
    good.cells["hp"] = ci(30); good.cells["name"] = ct("Ami");
    revive(ami, good, *ReloadPolicy);

    std::cout << "\n  death 2 — a corrupted save tries to return:\n";
    loom::Value corrupt; corrupt.schema = PlayerState;     // it still CLAIMS to be PlayerState
